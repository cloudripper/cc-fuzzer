"""cc_fuzzer_core.enums — single source of truth (SSOT) for every cc-fuzzer state enum.
...
Two interfaces, one definition:
  ...
  Python:  from cc_fuzzer_core.enums import CATEGORIES, EXPLOITABILITY, ...
           (scripts/_lib/enums.py is a re-export shim, so the plugin's
            `import enums` sibling imports keep working unchanged.)

  CLI:     cc-fuzzer enums print <name> [--sep <s>]    # members, newline-sep
           cc-fuzzer enums check <name> <value>        # exit 0 if member else 1
           cc-fuzzer enums doc-drift [STATE_SCHEMA.md]
           (python3 scripts/_lib/enums.py print|check|doc-drift is the same
            CLI, via main() below.)
...
def _default_schema_doc():
    from cc_fuzzer_core.paths import data
    return str(data("STATE_SCHEMA.md"))


def main(argv):
    """The legacy enums.py CLI (argv without the program name). Messages and
    exit codes are the contract scripts/_lib/enums.py callers rely on."""
    if len(argv) >= 1 and argv[0] == "doc-drift":
        # Default: the data root's schema doc (paths.data: $CC_FUZZER_ROOT,
        # the installed package data, or this checkout).
        path = argv[1] if len(argv) > 1 else _default_schema_doc()
...
_main = main  # pre-move name


# ---------------------------------------------------------------------------
# cc-fuzzer enums <verb>
# ---------------------------------------------------------------------------

def register_cli(subparsers):
    from cc_fuzzer_core.cli import add_subsystem

    _p, verbs = add_subsystem(subparsers, "enums", "state enum vocabularies (the SSOT)")
    v = verbs.add_parser("print", help="print an enum's members (sorted)")
    v.add_argument("name")
    v.add_argument("--sep", default="\n", help="separator (default: newline)")
    v.set_defaults(func=lambda a: main(["print", a.name, "--sep", a.sep]))
    v = verbs.add_parser("check", help="exit 0 if value is a member, else 1")
    v.add_argument("name")
    v.add_argument("value")
    v.set_defaults(func=lambda a: main(["check", a.name, a.value]))
    v = verbs.add_parser("doc-drift", help="compare STATE_SCHEMA.md's enum mirrors with the SSOT")
    v.add_argument("path", nargs="?")
    v.set_defaults(func=lambda a: main(["doc-drift", *([a.path] if a.path else [])]))
    v = verbs.add_parser("list", help="list the known enum names")
    v.set_defaults(func=lambda _a: (sys.stdout.write("\n".join(sorted(_REGISTRY)) + "\n"), 0)[1])
