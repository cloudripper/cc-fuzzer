"""cc_fuzzer_core.state.ceiling (was scripts/_lib/ceiling_probe.py) — the deterministic "is this a real coverage ceiling?" probe.
...
Pure: `compute()` writes nothing. probe(campaign) (`cc-fuzzer state ceiling-probe`,
via the ceiling-probe.sh shim) writes a `ceiling-probe/v1` snapshot for audit + the
pre-halt consult briefing.
"""
...
def _roundup_gain(snaps_dir, since_ts):
    ...
    series = roundup_series(snaps_dir, since_ts)
    gain_ts = last_gain_ts(series, since_ts)
    return gain_ts, ticks_since(series, gain_ts)


@dataclass(frozen=True)
class CeilingResult:
    path: Path      # the ceiling-probe-<now>.json snapshot written
    block: dict     # the computed block (without the snapshot's timestamp)


def probe(c: Campaign, current_path=None) -> CeilingResult:
    """Compute the probe against current.json + the campaign state and write a
    ceiling-probe/v1 snapshot. FileNotFoundError when there is no current.json."""
    from cc_fuzzer_core.state.yolo_state import YoloSettings

    cur_path = Path(current_path) if current_path else c.state_dir / "current.json"
    ...
    enabled_at_ts = ys.int("enabled_at_ts")
    ...
    out_ts = dict(out)
    out_ts["timestamp"] = now
    dest = os.path.join(snaps_dir, f"ceiling-probe-{now}.json")
    tmp = dest + ".tmp"
    with open(tmp, "w") as f:
        json.dump(out_ts, f, indent=2)
    os.replace(tmp, dest)
    return CeilingResult(Path(dest), out)
