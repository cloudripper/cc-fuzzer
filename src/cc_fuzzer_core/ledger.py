...
evaluator also billed the tokens on `tick` rows (a double count). Now:
  ...
  append(campaign, *, agent, usage, source, call_id)
      writes one `agent_call` event (event/v1 plus `source`, `call_id`, and
      the cache/model fields when known). Idempotent on call_id: a host hook
      and a driver reporting the same call write one row.
...
class Appended:
    row: dict
    appended: bool   # False: call_id already recorded, `row` is the earlier one
...
def _fields(agent: str, usage: Usage, source: str, call_id: str | None) -> dict:
...
def append(campaign, *, agent: str, usage: Usage, source: str, call_id: str | None = None,
           now: float | None = None) -> Appended:
    """Record one agent call. `campaign` is a Campaign or a state dir.
    Host sources must name the call (call_id); an orchestrator row may not."""
    ...
    with events.locked(_state_dir(campaign)) as log:
        if call_id:
            for r in log.rows:
                if r.get("event") == EVENT and r.get("call_id") == call_id:
                    return Appended(r, False)
        return Appended(log.append(EVENT, _fields(agent, usage, source, call_id), now=now), True)


def append_fields(campaign, *, agent_called: str = "", tokens_in=0, tokens_out=0, cache_read=0,
                  cache_write=0, model=None, source: str = ORCHESTRATOR, call_id=None, **_ignored) -> Appended:
...
DUPLICATE, SUPERSEDED, COUNTED = "duplicate", "superseded", "counted"


def classify(rows) -> list[tuple[dict, str]]:
    """[(agent_call row, status)] in log order: `duplicate` (a call_id seen
    earlier), `superseded` (an orchestrator row for an agent+tick that has a
    host-sourced row) or `counted`. Non-agent_call rows are skipped."""
...
def dropped(rows) -> set:
    """id()s of the agent_call rows spend ignores (duplicates, superseded) --
...
class Spend:
    ...
    def as_dict(self) -> dict:
        ...
        return {
            ...
            "superseded": self.superseded, "duplicates": self.duplicates,
        }
...
# ---------------------------------------------------------------------------
# CLI: cc-fuzzer ledger append | spend | show
# ---------------------------------------------------------------------------
...
def _cmd_append(a):
    ...
    if a.json:
        ...
        print(f"ledger: call_id {a.call_id} already recorded (tick {r.row.get('tick')}); nothing written")
    return 0
...
def _cmd_spend(a):
    ...
    if sp.superseded or sp.duplicates:
        print(f"ignored: {sp.superseded} superseded orchestrator row(s), {sp.duplicates} duplicate call_id row(s)")
...
def _cmd_show(a):
    ...
    if c is None:
        return code
    rows = classify(events.read(c.state_dir))
    if a.json:
...
def register_cli(subparsers):
    ...
    v = verbs.add_parser("append", help="record one agent call (idempotent on --call-id)")
