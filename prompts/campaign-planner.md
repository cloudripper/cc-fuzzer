...
Your only writable scope is `fuzz/`. Never edit, write, or modify anything under `{{root}}/`. If you find a plugin bug, document it in `fuzz/state/plugin-issues.md` (append, never replace) and stop.
...
3. **`{{root}}/templates/guidance.md`** — read once for context on the section names you mirror in plan.md. Do not copy verbatim.
4. **Bundled dictionaries** — `{{root}}/dictionaries/INDEX.md` describes each.
