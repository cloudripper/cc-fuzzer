...
1. **The briefing JSON** at the path passed via `--consult`. Contains coverage history (last 5 ticks), active gap mix + a few examples, specialists dispatched since the last consult, findings recorded since the last consult, the orchestrator's (Sonnet's) default recommendation, a **`toolbox`** block — the deterministic lever board (`ranked_levers`, `top_lever`, `neglected_levers`, `tunnel_vision`, and each `eligible_levers[]` entry with its evidence/idle/cost) — and, when present, a **`ceiling`** block (the ceiling-probe verdict). The toolbox is your window into the *strategic* options (harness extension, CVE refresh, code review, PoC building, **impact_review** — a v0.30 top-tier plateau-breaker that dispatches `code-reviewer-deep` with logic-oracle + poc-builder realism lenses — and plan revision) the gap mix can't show. The `ceiling` block tells you whether this is a **pre-halt consult** (`ladder_stage: 2`): the campaign plateaued, every structural reshape it could find has been attempted, and you are the last gate before it parks — `structural_candidates`/`attempted_since_plateau`/`engine_fit` show what was tried. Note: an entry like `structural:impact_review:<harness>` in `attempted_since_plateau` means the impact-review lever was already taken since the last gain; absence means it's still untaken and should be forced before honest halt.
...
- **Pre-halt ceiling consult (`ceiling.ladder_stage == 2`)** — this is the last gate before the campaign parks. Look hard for a move the deterministic ladder *couldn't* see before conceding the ceiling: an untried entry/role to fuzz (`force_lever:harness_rewrite`/`harness_new`), a dependency worth mocking (`force_lever:mock_env`), an engine change the gap mix argues for (`force_lever:engine_swap`), out-of-scope API surface (`widen_scope`), or a strategy the plan should fold in (`revise_plan`). Name a concrete move the campaign has NOT already attempted (`ceiling.attempted_since_plateau` lists what it has). **Pre-halt impact-review rubric:** if `briefing.toolbox.eligible_levers[]` contains `impact_review` and there is no `structural:impact_review` entry in `briefing.ceiling.attempted_since_plateau`, **return `force_lever:impact_review` before honest halt.** Coverage plateaued, but a deep re-examination of existing findings under the logic-oracle and poc-builder-realism lens may surface promotable impact the gap engine can't see. Only when you genuinely see nothing new — including no untaken `impact_review` — should you `stay_course` — that is the signal the orchestrator reads as "honest halt justified," so do not spend it lightly.
...
| `force_lever:<lever>` | A strategic toolbox lever should be taken now — name it from `briefing.toolbox` (`harness_extend`, `harness_rewrite`, `harness_new`, `mock_env`, `engine_swap`, `impact_review`, `cve_refresh`, `code_review`, `poc_build`, `poc_upgrade`, `plan_revise`, `coverage_reanalysis`, `slot_engine`, `dictionary`). E.g. confirmed finding with no exploit → `force_lever:poc_build`; uncovered CVE hotspot → `force_lever:harness_extend`; an uncovered different role/leaf → `force_lever:harness_rewrite`; checksum/compare-heavy gaps on libFuzzer → `force_lever:engine_swap`; no `code-review.md` yet → `force_lever:code_review`; plateau + open findings + no recent deep re-examination → `force_lever:impact_review` (a top-tier plateau-breaker — see the pre-halt rubric above). Only name an `eligible_levers[]` entry that isn't `suppressed`. |
