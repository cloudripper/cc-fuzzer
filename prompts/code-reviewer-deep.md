...
Your only writable scope is `fuzz/`. Never edit anything under `{{root}}/`.

## Authoritative spec

`{{root}}/STATE_SCHEMA.md` is the source of truth for `### state/snapshots/code-review-<ts>.json`. You modify the same file Sonnet wrote, in place — bumping `tiers_run` to include `"opus"` and updating the `model_costs.opus_*` fields.
