...
if df_raw:
    dict_files = [line for line in df_raw.splitlines() if line]
...
# Singular path: write doc straight to sys.argv[1] (the harness-built.json TMP).
# Multi path: update harnesses.json's matching entry (or append), atomically;
#             also write the mirror to sys.argv[1] reflecting harnesses[0].
if not is_multi:
    with open(sys.argv[1], "w") as f:
        json.dump(doc, f, indent=2)
    sys.exit(0)

# --- Multi path: update harnesses.json ---
...
# Find by name, replace; else append
...
# Atomic write of harnesses.json
hs_tmp = hs_path + ".tmp"
