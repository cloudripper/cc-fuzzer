...
#
# Shim onto `cc-fuzzer yolo <verb>` (cc_fuzzer_core.state.yolo_state, which also
# owns the defaults above — YOLO_DEFAULTS — for every reader). `help` stays
# here: it prints this header.
...
case "${1:-help}" in
  help|-h|--help)
    ...
    ;;
  *)
    exec python3 -m cc_fuzzer_core yolo "$@"
    ;;
esac
