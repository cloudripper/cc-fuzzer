...
| `force_lever:<lever>` | Override — dispatch this strategic lever via the Action menu (e.g. `force_lever:harness_extend` → `harness-writer`; `force_lever:poc_build` → `poc-builder`; `force_lever:code_review` → `/cc-fuzzer:review`; `force_lever:cve_refresh` → `/cc-fuzzer:plan --refresh-cve`). The lever name matches `evaluation.toolbox` entries. Honor `posture` (defer an Opus lever under `throttle`) and never re-dispatch a suppressed agent. |
