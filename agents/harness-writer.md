...
this environment has. `${CLAUDE_PLUGIN_ROOT}/bin/cc-fuzzer variants list` shows the declarations;
`${CLAUDE_PLUGIN_ROOT}/bin/cc-fuzzer variants spec --harness <name>` resolves them for this harness, including
...
```bash
${CLAUDE_PLUGIN_ROOT}/bin/cc-fuzzer build plan --harness <name> --backend <backend>    # the exact commands, runs nothing
```
...
The builder adds `-fno-sanitize-recover` for these; check with `${CLAUDE_PLUGIN_ROOT}/bin/cc-fuzzer build plan --harness <name>`.
